...
export default function EntrepriseCalendar() {
  ...
  const [ugcCreators, setUgcCreators] = useState<{ _id: string; name: string }[]>([]);
  ...
  const fetchEvents = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/calendar/entreprise/events');
  ...
  const fetchOffers = async () => {
    try {
      const response = await fetch('/api/entreprise/offers');
  ...
  const fetchUgcCreators = async () => {
    try {
      const response = await fetch('/api/entreprise/ugc-creators');

      if (!response.ok) {
        console.warn("L'API des créateurs UGC n'est pas disponible:", response.status);
        setUgcCreators([]);
        return;
      }
  ...
  const handleDeleteEvent = async (eventId: string) => {
    try {
      ...
      const response = await fetch(`/api/calendar/entreprise/events/${eventId}`, {
        method: 'DELETE',
      });
    ...
    <div>
      ...
      <div className="relative isolate px-4 pt-5 lg:px-8 mb-40">
        <div className="flex items-center mb-6">
          <Link href="/entreprise/dashboard" className="mr-4">
            <button className="bg-gray-200 rounded-full p-2">
              <FontAwesomeIcon icon={faArrowLeft} className="text-gray-600" />
            </button>
          </Link>
          <h1 className="text-2xl font-bold text-gray-900">Calendrier des campagnes</h1>
        </div>
                ...
                <div className="overflow-y-auto max-h-[80px]">
                  {day.events.map((event) => (
                    ...
                    >
                      ...
                      {event.title}
                      {event.ugcName && <span className="italic block">{event.ugcName}</span>}
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              ...
              <div className="space-y-4">
                ...
                {showEventDetails.ugcName && (
                  <div>
                    <p className="text-sm text-gray-500">Créateur de contenu:</p>
                    <p className="text-gray-700 font-medium">{showEventDetails.ugcName}</p>
                  </div>
                )}
